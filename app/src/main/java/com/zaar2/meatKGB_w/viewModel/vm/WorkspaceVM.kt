package com.zaar2.meatKGB_w.viewModel.vm
...
import com.zaar2.meatKGB_w.model.mappers.remoteToDb.RecordMapperRemoteToDb
import com.zaar2.meatKGB_w.model.mappers.dbToUi.RecordMapperDbToUi
import com.zaar2.meatKGB_w.model.mappers.uiToRemote.RecordMapperUiToRemote
...
class WorkspaceVM(myContext: Context): BaseVM(myContext) {
    ...
    fun addRecord(recordUi: RecordUi) {
        viewModelScope.launch {
            ...
            if (sessionId.isNotEmpty()) {
                if (isOnline()) {
                    val recordApi = RecordMapperUiToRemote(myContext).executeSuspend(recordUi)
                    ...
                    if (!recordsApi.isNullOrEmpty()) {
                        val recordsDb = recordsApi.let { RecordMapperRemoteToDb().execute(it) }
                        val arrIds =
    ...
    fun delete(idRecord: Long) {
        viewModelScope.launch {
            ...
            var loginUser = ""
            var idEnterprise = ""
            val recordsApi: List<RecordRemote>?
            val bundle = SharedPreferencesRepositoryImpl(myContext).getPreferencesVal(
                arrayOf(
